...
var goodsProUtils = {
	...
	parseJson : function (){
		...
		$.each(this.resultJson._ReturnData, function(index, pk){
			...
			content += "	<span>" + pk.propertyName + "：</span><input type='hidden' value='" + pk.tPropertyKeyId + "'/><select = 'select-sku'>";
		...
		$.each(skus, function(j, sku){
			...
			if (sku.skuIdStr == "NONE") {
				...
				if (parseFloat(_this._salePrice) > 0) {
					content += "		<div class='half'><span>销售价：</span><input type='number' value='" + sku.saleprice + "' readonly></div>";
				} else {
					content += "		<div class='half'><span>销售价：</span><input type='number' value='" + sku.saleprice + "'></div>";
				}
				content += "		<div class='half'><span>市场价：</span><input type='number' value='" + sku.marketprice + "'></div>";
				content += "	</div>";
				content += "	<div class='item clearfix'>";
				content += "		<div class='half'><span>库存：</span><input type='number' value='" + sku.skustock + "'></div>";
				content += "	</div>";
				content += "</div>";
