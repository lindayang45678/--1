...
public class UserServiceImplBAK {
	...
	public ObjectOutput<UserInfoOutput> login(UserLoginInput user) throws LakalaException {
		...
		if (null != mediaStatistics && mediaStatistics.size() > 0
			...
			if (null != tmember) {
				//检查用户是否已经登录(用户登出的时候，清空终端编码)                                     次逻辑删除掉，暂时不用单手机登录
			...
			if (null != mediaStatistics && mediaStatistics.size() > 0
				...
				if (null != tmember) {
					...
					try {
						...
						uid = (uid == null ? 0L : uid);
						if (Long.compare(uid, 0L) > 0) {
							data.set_ReturnMsg("该手机号已激活，无需重复激活");
							data.set_ReturnCode(ReturnMsg.CODE_USER_100014);
							return data;
						}
						//RSA解密
						String plainpwd = RSA.decrypt(user.getPwd());
						...
						if (status == LoginStatusEnum.LOGINSUCCESS) {
							//集团用户中心服务
							Long gid = userWebService.createUserIfNotExist(user.getMobile(), plainpwd, SYSTEM_CODE);
							gid = (gid == null ? 0L : gid);
				...
				try {
					...
					if (Long.compare(gid, 0L) > 0) {
						...
						if (rtn > 0) {
							...
							//为ToC添加新用户
//							mapC.put("mobile", user.getMobile());
//							mapC.put("pwd", enpwd);
//							mapC.put("groupId", gid);
//							tmemberMapper_W.addTmemberC(mapC);
