...
public class OrderController {
	 ...
	 * @todo: TODO
	 */
	@RequestMapping(value = "/entrycnt",method = RequestMethod.POST)
	public @ResponseBody ObjectOutput orderEntryCnt(OrderCntInput orderentryinput){
		ObjectOutput info = null;
		try{
			info = orderService.orderEntryCnt(orderentryinput);
		}catch(LakalaException e){
			logger.error(e.getMessage());
			info = new ObjectOutput();
			info.set_ReturnCode(ReturnMsg.CODE_ERR_DEFAULT);
	...
	public @ResponseBody ObjectOutput orderEntryList(OrderInfoInput orderinfoinput){
		ObjectOutput info = null;
		try{
			info = orderService.orderEntryList(orderinfoinput);
		}catch(LakalaException e){
			logger.error(e.getMessage());
			info = new ObjectOutput();
			info.set_ReturnCode(ReturnMsg.CODE_ERR_DEFAULT);
	 ...
	 * @return
	 * @author: yhg 
	 * @time: 2015年4月17日 上午11:17:41
	 * @todo: TODO
	 */
	@RequestMapping(value = "/entrylistv2",method = RequestMethod.POST)
	public @ResponseBody ObjectOutput orderEntryList(OrderV2InfoInput orderv2infoinput){
		ObjectOutput info = null;
		try{
			info = orderService.orderEntryListV2(orderv2infoinput);
		}catch(LakalaException e){
			logger.error(e.getMessage());
			info = new ObjectOutput();
			info.set_ReturnCode(ReturnMsg.CODE_ERR_DEFAULT);
	 ...
	 * @return
	 * @author: yhg 
	 * @time: 2015年4月27日 上午9:39:08
	 * @todo: TODO
	 */
	@RequestMapping(value = "/cancelorder",method = RequestMethod.POST)
	public @ResponseBody ObjectOutput cancelorder(OrderDetailInput orderdetailinput){
		ObjectOutput info = null;
		try{
			info = orderService.updatecancelorder(orderdetailinput);
		}catch(Exception e){
			logger.error(e.getMessage());
			info = new ObjectOutput();
			info.set_ReturnCode(ReturnMsg.CODE_ERR_DEFAULT);
	 ...
	 * @return
	 * @author: yhg 
	 * @time: 2015年4月27日 下午2:53:42
	 * @todo: TODO
	 */
	@RequestMapping(value = "/confirmorder",method = RequestMethod.POST)
	public @ResponseBody ObjectOutput confirmorder(OrderDetailInput orderdetailinput){
		ObjectOutput info = null;
		try{
			info = orderService.updateconfirmorder(orderdetailinput);
			if(!StringUtils.isEmpty(orderdetailinput.getTorderproviderid())){
				orderService.msgwhenconfim(orderdetailinput.getTorderproviderid());
			}
