...
public class WholesaleController {
    ...
    public ObjectOutput detail_mongo(InputModel inputModel) {
        ObjectOutput info = null;
        try {
            info = sideShopBMongoService.detail_mongo(inputModel.getPsam(), inputModel.getChannelid(), inputModel.getGoodsid(), inputModel.getVirtualcatid(), Integer.parseInt(inputModel.getLevel() == null ? "0" : inputModel.getLevel()));
        } catch (LakalaException e) {
            logger.error(e.getMessage());
            info = new ObjectOutput();
            info.set_ReturnCode(ReturnMsg.CODE_ERR_DEFAULT);
