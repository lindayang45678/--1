...
public class UserServiceImpl implements UserService{
	...
	public ObjectOutput<UserInfoOutput> saveUserRegister1(UserRegisterInput user)
		...
		//debug
		logger.info("saveUserRegister->mobile:" + user.getMobile());
...
//			}
		...
		if (Long.compare(uid, 0L) > 0) {
			...
			try {
				...
				linetext.append("\n");
				logger.info(linetext.toString());
			} catch (Exception e) {
				logger.error("开通手机钱包失败：" + e.getMessage());
			...
			try {
				...
				params.put("psam", psam);
				String resultMap = HttpSend.post(wxService.getUrl_ll(), params);
				//调用振华的接口，由振华来操作数据库，添加地理位置信息 yangjunguo 2015-07-31
/*				String lng = "";
				String lat = "";
