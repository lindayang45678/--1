...
var ordercate = "all";  //大类  批发、零售（lsdd）、需店主送货
...
function getretailordercnt(ordercate) {
	...
	if(resultJson._ReturnCode === returnCodeSuccess) {
		...
		result += "</a></li>";
		result += "<li><a id='lsyfh' href='javascript:;' class='f-14'><span>待收货</span>";
		if(parseInt(ls2deliverednum) > 0) {
			result += "<i>" + ls2deliverednum + "</i>";
		}
		result += "</a></li>";
		result += "<li><a id='lsyqs' href='javascript:;' class='f-14'><span>已收货</span>";
		...
		for(var j = 0; j < orderproviderlist.length; j ++) {
			...
			for(var k = 0; k < orderitemslist.length; k ++) {
				...
				if(paychanel == paychanelCod) {
					result += "<div class='cash-delivery'>&nbsp;</div>";
				}
				
				if(state == stateYQS && querytype=="lsall") {
					result += "<div class='order-complete'>&nbsp;</div>";
				}
				
				if((cancelstate == cancelstateY || state == stateYJS ) && querytype=="lsall") {
					result += "<div class='order-cancel'>&nbsp;</div>";
				}
...
function eleBindEvent() {
	...
	$(".orders-search input").on("keyup",function(e) {
		if(e.keyCode == 13) {
			...
			$(".orders-search input").val("");
			window.location.href = encodeURI(encodeURI("querysuccess.html?querytype=lsall&searchvalue=" + searchvalue + "&t=" + t));
		}
	});
	//点击搜索事件
