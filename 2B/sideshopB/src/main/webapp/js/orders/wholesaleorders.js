...
function eleBindEvent() {
	...
	$(".topayall").live("tap",function() {
		...
		/*if(source == sourceSbApp2BWebchat) {
			showAlertMsg(msgPayForWebchat);
			return;
		}*/
	...
	$(".orders-search input").on("keyup",function(e) {
		if(e.keyCode == 13) {
			...
			$(".orders-search input").val("");
			window.location.href = encodeURI(encodeURI("querysuccess.html?querytype=pfall&searchvalue=" + searchvalue + "&t=" + t));
		}
	});
	//点击搜索事件
