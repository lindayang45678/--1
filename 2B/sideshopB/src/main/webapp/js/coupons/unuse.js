			...
			if(resultJson._ReturnCode === returnCodeSuccess) {
				...
				if(couponList.length > 0) {
					for (var i = 0; i < couponList.length; i++) {
						var url = encodeURI(encodeURI("details.html?favorablecode="+ couponList[i].favorablecode + "&startflag="+ couponList[i].startflag + "&couponType="+ couponList[i].couponType +"&cost="+ couponList[i].cost +"&orderamount="+ couponList[i].orderamount + "&usednum=" + couponList[i].usednum + "&usenum=" + couponList[i].usenum + "&endtime=" + couponList[i].endtime + "&frequencys=" + couponList[i].frequencys +""));
