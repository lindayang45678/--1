...
$(function(){
	...
	$('.choi-sort').on('tap', function(){
		if (writeGoodsInfo.catEnable) {
			$("#schose-choi-sort").removeClass('hidden');
			$("#add-choi-sort").addClass('hidden');
	...
	$(".head-right").on("touchend", function (e){
		...
		$("input:focus").blur();
		e.preventDefault();
		setTimeout(function(){submitAddGoods();},300);
	});
	//ios下fixed定位失效
	osFixed();
	//上传图片按钮事件绑定
...
var goodsInfo = {
	...
	_requestData : function (){
		...
		"trealcatedisc" : this.trealcatedisc, "trealcatetreedisc" : this.trealcatetreedisc, "mobile" : this._mobile,
		...
		"deviceno" : this.deviceno, "virtualcateids" : this.virtualcateids, "tGoodInfoPoolId" : this.tGoodInfoPoolId,
		"isModel" : this._isModel,"isfreshfood" : this.isfreshfood}
	}
}
...
function invokePhoto(){
	...
	cordova._native.camera.picker({width:300, height:300, quality:.5}, function(res){
	    ...
	    //调用图片上传接口，需要写Timage表
		var data = {"token" : userToken, "isWriteTimage" : "1", "data":res.data, "fileName" : res.path, "osType" : osType(), "mobile" : mobile, "targetType" : 1};
		...
		if(resultJson._ReturnCode === returnCodeSuccess) {
			...
			$.each(resultJson._ReturnData.success, function(index, img){
				...
				//拼删除按钮
				$("#" + img.split("_")[0]).find("img").after("<del></del>");
			});
...
var goodsProUtils = {
	...
	isfreshfood : 0, //生鲜标记，默认非生鲜

	//初始化
	init : function (trealcateid, isfreshfood){
	...
	parseJson : function (){
		...
		$.each(this.resultJson._ReturnData, function(index, pk){
			...
			content += "</select><em class='edit'></em></div>";
			if (index == _this.resultJson._ReturnData.length - 1 || index % 2 == 1) {
				content += "	</div>"
			}
		});
		content += "	<div class='item clearfix'>";
		content += "		<div class='whole1 sele'><span>销售价：</span><div><input name='saleprice' type='number' min='0' step='0.01' value=''><em class='edit'></em></div></div>";
		if (_this.isShowStock()) {
			content += "		<div class='whole1 sele'><span>库存：</span><div><input name='stock' type='tel' value=''><em class='edit'></em></div></div>";
		}
		content += "	</div>";
		//自定义商品编辑时，去掉“添加规格”--20150615
		//content += "	<div class='item clearfix'>";
		//if (this.resultJson._ReturnData.length != 0) {
		//	content += "		<div class='whole1 " + (i > 0 ? "del'>删除规格" : "add'>添加规格") + "</div>";
		//}
		//content += "	</div>";
		content += "</div>";
	...
	selectedProperties : function (){
		...
		$.each(ggs, function (index, gg){
			//获取规格的属性集合
//			var pros = $($(gg).find(".item")[0]).find(".whole1");
			var pros = $(gg).find(".whole1");
			var kv_id = "";//属性、属性值键值对
			var v_disc = "";//属性值描述
			...
			//销售价
			var salePrice = $(gg).find("input[name='saleprice']").val();
			if(salePrice.indexOf(".") > 0) {
				//保留两位小数，不采用四舍五入
				salePrice = salePrice.substring(0, salePrice.indexOf(".")+3);
			}
			//市场价（后期要求页去掉市场价，为了处理方便，这里市场价直接赋值为0）
			var marketPrice = 0;
			//库存
			var stock = $(gg).find("input[name='stock']").val();
			//stock = undefined时，该商品为自定义商品，或模板非生鲜商品，不更新库存，
			//约定：传入后台时，如果库存为0.00，则不更新库存字段
		...
		$.each(skus, function(j, sku){
			...
			if (sku.skuIdStr == "NONE") {
				...
				if (parseFloat(_this._salePrice) > 0) {
					content += "		<div class='whole1 sele'><span>销售价：</span><div><input name='saleprice' type='number' min='0' step='0.01' value='" + sku.saleprice + "' readonly><em class='edit'></em></div></div>";
				} else {
					content += "		<div class='whole1 sele'><span>销售价：</span><div><input name='saleprice' type='number' min='0' step='0.01' value='" + sku.saleprice + "'><em class='edit'></em></div></div>";
				}
				if (_this.isShowStock()) {
					content += "		<div class='whole1 sele'><span>库存：</span><div><input name='stock' type='tel' value='" + sku.skustock + "'><em class='edit'></em></div></div>";
				}
				content += "	</div>";
				content += "</div>";
			...
			$.each(_this.resultJson._ReturnData, function(index, pk){
				...
				content += "<div class='whole1 sele'>";
				content += "	<span>" + pk.propertyName + "：</span><input type='hidden' value='" + pk.tPropertyKeyId + "'/><select class='select-sku' disabled='disabled'>";
				content += "<option >不选择</option>";
				//遍历属性值
				$.each(pk.propertyValues, function(_index, pv){
			...
			//自定义商品编辑时，去掉“添加规格”--20150615
			//content += "	<div class='item clearfix'>";
			//content += "		<div class='whole1 " + (j != 0 ? "del'>删除规格" : "add'>添加规格") + "</div>";
			//content += "	</div>";
			content += "</div>";
		});
		return content;
...
var writeGoodsInfo = {
	...
	getGoodsData : function (goodsid){
		if (!isNullString(goodsid)) {
			//调用接口
			var data = {"token" : userToken, "goodsid" : goodsid, "mobile" : mobile};
			var resultJson = ajaxCommon(urlselfgoodsdetail, data);
			resultJson = convertNullToEmpty(resultJson);
			edit = resultJson._ReturnData.tGoodInfoPoolId;
			if (edit) {
				$(".editline-n em").remove();
				$(".edit-line1 em").remove();
			}
			//解析结果
			if(resultJson._ReturnCode === returnCodeSuccess) {
	...
	parseBaseData : function (resultJson){
		//判断是否是平台商品
		if (!isNullString(resultJson._ReturnData.tGoodInfoPoolId)) {
			isModel = true;
		}
		
		goodsInfo.tgoodinfoid = resultJson._ReturnData.tGoodInfoId;
		//商品名称
		...
		goodsProUtils.init(tRealCateId, resultJson._ReturnData.isfreshfood);
		//初始商品图片数据
		...
		goodsInfo.tGoodInfoPoolId = resultJson._ReturnData.tGoodInfoPoolId;
		//禁用分类选择插件
		if (!isNullString(goodsInfo.tGoodInfoPoolId)) {
	...
	parseImgData1 : function (imgs){
		var content = "";
		$.each(imgs, function (index, img){
			content += "<li id='" + img.timageId + "'><img src='" + img.url + "'/>" + (isModel? "" : "<del></del>") + "</li>";
		});
		return content;
	},
...
function submitAddGoods(){
	...
	goodsInfo.goodname = $(".nonstock-part2").find(".max-3").text();
	goodsInfo.goodintroduce = $(".nonstock-part2").find(".max-n").text();
	//获取页面选中的sku
	goodsInfo.skuidstr = goodsProUtils.selectedProperties();
	//获取页面的图片
	...
	if (isNullString(goodsInfo.skuidstr)) {
		...
		$.each(temp, function(index, t){
			...
			if (temp.length > 1) {
				...
				if (_temp[4] != "0.00" && (isNullString(_temp[4]) || isNaN(_temp[4]) || parseFloat(_temp[4]) <= 0) && !arrNoReapet(msg, "库存必填，且必须为大于0的数字！")) {
	...
	//解析结果
	if(resultJson._ReturnCode === returnCodeSuccess) {
		if (isNullString(goodsInfo.tGoodInfoPoolId)) { //自定义商品
			showAlertMsg("保存成功，拉卡拉审核通过后，可上架销售！", "index.html?back=index");
		} else { //模板商品
			showAlertMsg("保存成功，该商品已上架销售！", "index.html?back=index");
		}
	} else {
		$(".head-right").removeClass("clicked");
		showAlertMsg(resultJson._ReturnMsg);
	}
}
...
function getPayAndDelivery() {
	...
	//获取支付方式
	if (isHomepay == "1") {
		goodsInfo.ispayafterarrival = 1;
	} else {
		goodsInfo.ispayafterarrival = 0;
	}
	
	//获取配送方式
	if (homeDeliver == "1" && isPickup == "1") {
		goodsInfo.isexpresstohome = 0;
	} 
	if (homeDeliver != "1" && isPickup == "1") {
		goodsInfo.isexpresstohome = 1;
	} 
}
