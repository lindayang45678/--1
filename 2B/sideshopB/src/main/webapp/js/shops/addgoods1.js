...
$(function(){
	...
	//选择分类
	$('.choi-sort').on('click', function(){
		if (writeGoodsInfo.catEnable) {
			$("#schose-choi-sort").removeClass('hidden');
			$("#add-choi-sort").removeClass('hidden').css("pointer-events","none");
...
function removePhoto() {
	$('.nonstock-part1 del').live('tap',function(){
		$(this).parent().remove();
	});
}
...
var goodsProUtils = {
	...
	parseJson : function (){
		...
		$.each(this.resultJson._ReturnData, function(index, pk){
			...
			content += "	<span>" + pk.propertyName + "：</span><input type='hidden' value='" + pk.tPropertyKeyId + "'/><div><select class='select-sku'>";
...
//			var pros = $($(gg).find(".item")[0]).find(".whole1");
			...
			//stock = undefined时，该商品为自定义商品，或模板非生鲜商品，不可维护库存，默认库存1000000
		...
		$.each(skus, function(j, sku){
			...
			if (sku.skuIdStr == "NONE") {
				...
				if (parseFloat(_this._salePrice) > 0) {
					content += "		<div class='whole1 sele'><span>销售价：</span><div><input name='saleprice' type='number' min='0' step='0.01' value='" + _this._salePrice + "' readonly><em class='edit'></em></div></div>";
				} else {
					content += "		<div class='whole1 sele'><span>销售价：</span><div><input name='saleprice' type='number' min='0' step='0.01' value=''><em class='edit'></em></div></div>";
				}
				if (_this.isShowStock()) {
...
function submitAddGoods(){
	...
	if(resultJson._ReturnCode === returnCodeSuccess) {
		if (isNullString(goodsInfo.tGoodInfoPoolId)) { //自定义商品
			showAlertMsg("保存成功，拉卡拉审核通过后，可上架销售！", "addgoods.html?back=index");
		} else { //模板商品
			showAlertMsg("保存成功，该商品已上架销售！", "addgoods.html?back=index");
		}
	} else {
