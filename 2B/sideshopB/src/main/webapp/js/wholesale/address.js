...
/* 添加地址超过5个的时候弹窗否则添加*/
$(".fill-add-address").on("click",function(){
...
function getShipAddress(){
	...
	content += "<em><i class='address-area'>" +shipprovince+ "&nbsp;" +shipcity+ "&nbsp;" +shiparea+ "</i></em>";
	...
	if (resultJson._ReturnCode === returnCodeSuccess) {
		...
		if(_ReturnData.length > 0){
			for (var i = 0; i < _ReturnData.length; i++) {
				...
				content += "<em><i class='address-area'>"+_ReturnData[i].shipprovince+ "&nbsp;"+_ReturnData[i].shipcity+"&nbsp;"+_ReturnData[i].shiparea+"</i></em>";
...
/* 添加收货人地址*/
$(".btn-save-addressa").on("click",function(){
	var shipname = $.trim($(".edit-name div").text());
	var shipaddr = $.trim($(".edit-address div").text());
	var shipmobile = $.trim($(".edit-phone div").text());
	if(shipname == "") {
		showAlertMsg(msgAddrdessShipNameNotEmpty);
		return;
	}
	if(shipaddr == "") {
		showAlertMsg(msgAddrdessNotEmpty);
		return;
	}
...
/* 编辑收货人地址*/
$(".btn-save-addressb").on("click",function(){
	var shipname = $.trim($(".edit-name div").text());
	var shipaddr = $.trim($(".edit-address div").text());
	var shipmobile = $.trim($(".edit-phone div").text());
	if(shipname == "") {
...
/* 删除收货地址*/
$(".fill-close").live("click",function(){
	$(this).parent().remove();
	var id = $(this).attr("id");
	var data = {
		"id":id	
...
function validatePhone(phone){
	phone = $.trim(phone);
	var reg= /^1[3,4,5,7,8]{1}\d{9}$/; //校验手机号
	if(phone == ""){
