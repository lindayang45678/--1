		...
		if (resultJson._ReturnCode === returnCodeSuccess) {
			...
			if(goodsSkuO2OList.length > 0){
				for(var i = 0 ;i < goodsSkuO2OList.length; i ++){
					...
					if(goodsSkuO2OList[i].type == promoTypeZhiJiang) {
						storeContent += "<div class='promo-price'>促销价<br/><i id='saleprice_o2o'>￥" +goodsSkuO2OList[i].promotionPrice+ "</i></div>";
						storeContent += "<div class='market-price'>批发价<br/><del id='marketprice'>￥" +goodsSkuO2OList[i].salePrice+ "</del></div>";
                    }else{
			...
			$("#propertyId").on("change",function(){
				...
				if(skuOption.eq(index).attr("promotype") == promoTypeZhiJiang) {
					...
					$("#pro-num").removeAttr("goodsname").removeAttr("promotype").removeAttr("purchasecount");
					$("#pro-title").html(goods.goodname);
				}
				//倒计时
