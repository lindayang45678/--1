...
/* 手动输入点击验证时候*/
$(".coupon-test-btn").live("click",function(event){
	couponlist = [];
	...
	if(resultJson._ReturnCode === returnCodeSuccess) {
		if(resultJson._ReturnData.retcode == 0000) {
			...
			$(".coupon-finish-btn").on("click",function(){
				storage.setItem("promfee", promfee);
				storage.setItem("couponlist", JSON.stringify(couponlist));
				window.location.href = "fill.html?t=" + t;
			});	
			...
			$(".coupon-cancel-btn").on("click",function(){
...
/* 点击手动和选择现金券进行切换*/
$(".select-coupon-tab").live("click",function(){
...
$(".hand-coupon-tab").live("click",function(){
	$(this).find("em").addClass("tab-red");	
...
function useCoupon(){
	...
	//优惠券加减号
	$(".pl-re").on("click",function(event){
		event.stopPropagation();		
	});
	$(".pluse").on("click",function(event){
...
/* 手动输入优惠券点击叉的时候清除*/
$(".coupon-use-x").on("click",function(event){
	$(".fill-coupon-use input").val("");
