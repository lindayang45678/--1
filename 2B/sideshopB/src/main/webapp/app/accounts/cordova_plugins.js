...
(function(){
     ...
     * 隐藏键盘
     */
    _native.navigation.hideKeyboard = function(){
        cordova.exec(null, null, "Navigation", "hideKeyboard", []);
    };
    ...
    _native.navigation.isWebGoBack = function(isTriggerByAndroidBackKey){
        _native.navigation.hideKeyboard();

        var isPaymentResult = false;
        if(window.location.href.indexOf('payfail.html') != -1){
            isPaymentResult = true;
        }
    ...
    _native.nativeinfo.get=function(success){
        cordova.exec(success,null,"NativeInfo","get",[]);
    };
})();
...
(function(){
    ...
    _native.system.deviceScreenSize = function (success) {
        cordova.exec(success,null,"System","deviceScreenSize",[]);
    }
    //---------  保存信息插件 ----------
    _native.saveInfo = {};
     ...
     * @param sting
     */
    _native.encrypt.md5Encrypt = function(string,success,failure){
        cordova.exec(success, failure, "Encrypt", "md5Encrypt", [string]);
    };
    /**
     * 加密sting
     *
     * @param array
     */
    _native.encrypt.md5EncryptArr = function(arr,success,failure){
        cordova.exec(success, failure, "Encrypt", "md5EncryptArr", [arr]);
    };

    _native.encrypt.pwdEncrpt = function(pwdString,success,failure){
        cordova.exec(success, failure, "Encrypt", "pwdEncrypt", [pwdString]);
    };
})();